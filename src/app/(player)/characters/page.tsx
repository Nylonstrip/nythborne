...
export default async function NythsPage() {
  ...
  const players = nyths?.filter((n: Character) => n.is_player_character) ?? []
  const npcs = nyths?.filter((n: Character) => !n.is_player_character) ?? []
      ...
      {players.length > 0 && (
        <section className={styles.section}>
          ...
          <div className={styles.grid}>
            {players.map((nyth: Character) => (
              <EntryCard
                key={nyth.id}
                href={`/characters/${nyth.id}`}
